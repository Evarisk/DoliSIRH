...
class Certificate extends CommonObject
{
	...
	public $fields=array(
		...
		'fk_societe'	=> array('type' => 'integer:Societe:societe/class/societe.class.php:1:status=1 AND entity IN (__SHARED_ENTITIES__)', 'label'=>'ThirdParty', 'enabled'=>'1', 'position'=>83, 'notnull'=>-1, 'visible'=>3, 'index'=>1, 'css'=>'maxwidth500 widthcentpercentminusxx',),
	...
	//  */
	// public $table_element_line = 'dolisirh_certificateline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_certificate';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Certificateline';
	...
	//  */
	// protected $childtablesoncascade = array('dolisirh_certificatedet');

	// /**
	//  * @var CertificateLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->dolisirh->certificate->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->dolisirh->certificate->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->dolisirh->certificate->certificate_advance->validate))))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->dolisirh->dir_output.'/certificate/'.$oldref;
				$dirdest = $conf->dolisirh->dir_output.'/certificate/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->dolisirh->dir_output.'/certificate/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->dolisirh->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->dolisirh->dolisirh_advance->validate))))
		 {
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'CERTIFICATE_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'CERTIFICATE_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/dolisirh/view/certificate/certificate_card.php', 1).'?id='.$this->id;
	...
	public function getLinesArray()
	{
		...
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_certificate = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->DOLISIRH_CERTIFICATE_ADDON)) {
			$conf->global->DOLISIRH_CERTIFICATE_ADDON = 'mod_certificate_standard';
		}

		if (!empty($conf->global->DOLISIRH_CERTIFICATE_ADDON)) {
			$mybool = false;

			$file = $conf->global->DOLISIRH_CERTIFICATE_ADDON.".php";
			$classname = $conf->global->DOLISIRH_CERTIFICATE_ADDON;

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/dolisirh/");
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			...
			} elseif (!empty($conf->global->CERTIFICATE_ADDON_PDF)) {
				$modele = $conf->global->CERTIFICATE_ADDON_PDF;
			}
		}

		$modelpath = "core/modules/dolisirh/doc/";
