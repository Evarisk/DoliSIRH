...
if ($enablepermissioncheck) {
	...
	$permissiondellink = $user->rights->dolisirh->certificate->write; // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->dolisirh->multidir_output[isset($object->entity) ? $object->entity : 1].'/certificate';
...
if (empty($reshook)) {
	...
	$triggermodname = 'DOLISIRH_CERTIFICATE_MODIFY'; // Name of trigger action code to execute when we modify record
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			$filedir = $conf->dolisirh->dir_output.'/'.$object->element.'/'.$objref;
			$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;
			$genallowed = $permissiontoread; // If you can read, you can build the PDF to read content
			$delallowed = $permissiontoadd; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('dolisirh:Certificate', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('certificate'));
