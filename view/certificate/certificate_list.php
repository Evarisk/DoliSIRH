...
/**
 *   	\file       certificate_list.php
 *		\ingroup    dolisirh
 *		\brief      List page for certificate
 */
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'certificatelist'; // To manage different context of search
...
if ($enablepermissioncheck) {
	...
	$permissiontodelete = $user->rights->dolisirh->certificate->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/dolisirh/view/certificate/certificate_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/dolisirh/view/certificate/certificate_card.php', 1).'?action=create', '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'dolisirh.png@dolisirh', 0, $newcardbutton, '', $limit, 0, 0, 1);
